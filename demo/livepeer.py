...
query_date_clause = '{timestamp_gte:%s,timestamp_lt:%s}' % (start_timestamp, end_timestamp)
query = """
{
...
""" % (query_date_clause, query_date_clause, query_date_clause)
...
df_amount = df_bond[['timestamp','amount','round.id']].append(df_rebond[['timestamp','amount','round.id']]).append(df_unbond[['timestamp','amount','round.id']]).reset_index()
df_amount_over_time = ts.aggregate_timeseries(df_amount, 
...
df_loss_gains = ts.aggregate_groupby(
    ...
    'transcoder',
    columns=[
        ts.ColumnConfig(name="loss", aggregate_method=ts.AggregateMethod.SUM, type=ts.ColumnType.float, na_fill_value=0.0),
        ts.ColumnConfig(name="gain", aggregate_method=ts.AggregateMethod.SUM, type=ts.ColumnType.float, na_fill_value=0.0)
    ]
